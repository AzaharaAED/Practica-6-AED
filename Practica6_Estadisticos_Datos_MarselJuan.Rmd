...
datos %>% 
  ...
  geom_density()+
  geom_vline(aes(xintercept = mean(valor)), color='red', size=1)#no se por que no va
...
t1 <- table(datos$ALG.cat)
t2 <- table(datos$ALG.cat, datos$ANM.cat)
...
datos %>% 
  mutate(raiz.num=sqrt(datos$ALG.num)) %>% 
  select(ends_with('.num')) %>% 
  cor(use='complete.obs', method='pearson')
...
```
Como p-value<0.05, podemos decir que existe una asociación estadísticamente significativa entre ALG.cat y ANM.cat. Es decir, la distribución de una de las variables depende de la otra.

```{r Solucion_Ej9b, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
chisq.test(datos$ALG.num,datos$Sex, correct=FALSE)
```
Como p-value>0.05, podemos decir que no parece haber una relación fuerte entre las variables ALG.num y Sex. Las diferencias observadas podrían deberse al azar, y no a una relación sistemática entre ambas.
